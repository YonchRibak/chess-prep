...
| [RashidPanel.tsx](../../apps/web/src/components/RashidPanel.tsx) | [Rashid](../03-domain/rashid.md) R3 live-probe panel in the editor, via `useRashid` ([lib/engine/useRashid.ts](../../apps/web/src/lib/engine/useRashid.ts)) — off by default (a probe costs seconds until R5 precompute), data only (arrows are R4), and runs on Rashid's dedicated engine, never the eval panel's singleton |
