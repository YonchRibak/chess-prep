...
| [router.test.ts](../../apps/web/src/lib/router.test.ts) | Flow F1: view ↔ hash round-trips for every view kind, including session scopes with spaces and interior colons; that a malformed `?scope=` param degrades to *absent* (stored rules apply) rather than nulling a valid route or inventing a scope. Flow F2: `guided=1` round-trips (alone and with a scope) and anything else parses un-guided. S5: `#/rehearse` round-trips with a chapter tag containing spaces, a colon and a slash, `?mode=expand` survives and any other mode parses as cards |
...
| [lineTransition.test.ts](../../apps/web/src/lib/chess/lineTransition.test.ts) | S5 transition planner: identical → none, continuation → extend, sibling → rewind to the common ancestor, shares-only-the-root → new line, a long rewind → new line even with a shared prefix, and a pure take-back to the root stays a rewind |
| [rehearse/queue.test.ts](../../apps/web/src/lib/rehearse/queue.test.ts) | S5 queue: tag scope keeps only that chapter's hero moves; unscoped = every live hero move (dropped/shadow excluded); deterministic shuffle under an injected rng; an uncarded hero move gets a **stub** new card while existing cards pass through untouched; a hero move below a demoted alternate is never stubbed nor queued, even with a stale card (every item's path length equals its depth); every item's `pathSans` replays (chess.js) to its parent FEN; a transposition is reached through the scoped chapter's own line (`prefer`) |
| [rehearse/chapterStats.test.ts](../../apps/web/src/lib/rehearse/chapterStats.test.ts) | S5 chapter rows: per-tag card / due / mastered counts; opponent and dropped moves ignored |
| [rehearse/expand.test.ts](../../apps/web/src/lib/rehearse/expand.test.ts) | S5 Expand variations: opponent-turn targets shallow-first with leaves, chapter filtering by the reaching edge, no descent into dropped branches; `uncoveredReplies` removes live, dropped **and** shadow SANs; source fallback explorer → engine → book → none with a cold explorer |
| [missFlow.test.ts](../../apps/web/src/lib/drill/missFlow.test.ts) | S5 extraction of the miss sequence: note stage only when the correct move has a non-blank comment; reveal plays → waits → takes back; an unplayable SAN is not undone; an abort mid-reveal leaves the board as it is |
